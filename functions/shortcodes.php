...
function shortcodes_page(){
	...
	<div class="wrap">
		...
		<ol>
			...
			<li>[business-hour] <span class="sdetagils">displays Business Hours from theme option</span></li>
...
function site_identity_func( $atts = array(), $content = null ) {
	...
	$logo_url = ($ainew_options['logo']['url']) ? $ainew_options['logo']['url'] : get_template_directory_uri(). '/images/logo.png';
	$logo_option = $ainew_options['logo-option'];
	$html = '';
	$atts = shortcode_atts( array(
		'class' => '',
		'container_class' => ''
...
function email_func( $atts = array(), $content = '' ) {	
	global $ainew_options;
	$contact_email = $ainew_options['contact-email'];
	$html = '';	
	$atts = shortcode_atts( array(
		'offset' => 0,
		'index' => 0,
...
function phone_func( $atts = array(), $content = '' ) {
	...
	if ($atts['index']) :
	    ...
	    $html .= preg_replace('/[^0-9]/', '', $ainew_options['contact-phone'][$i]);
	    $html .= '" >';
	    $html .= $ainew_options['contact-phone'][$i];  
	    $html .= '</a>';
	    $html .= '</span>';		
	else :
		if (@$ainew_options['contact-phone']) :
			foreach ($ainew_options['contact-phone'] as $phone) :
				if ($n > $atts['offset']) :
				    $html .= '<span class="phone-number">';
				    $html .= '<a class="phoneToShow" href="tel:';
...
function fax_func( $atts = array(), $content = '' ) {
	...
	if ($atts['index']) :
	    ...
	    $html .= preg_replace('/[^0-9]/', '', $ainew_options['contact-fax'][$i]);
	    $html .= '" >';
	    $html .= $ainew_options['contact-fax'][$i];  
	    $html .= '</a>';
	    $html .= '</span>';		
	else :
		if (@$ainew_options['contact-fax']) :
			foreach ($ainew_options['contact-fax'] as $fax) :
				if ($n > $atts['offset']) :
				    $html .= '<span class="fax-number">';
				    $html .= '<a class="faxToShow" href="tel:';
...
function business_hour_func( $atts = array(), $content = '' ) {
	$html = '';
	global $ainew_options;
	$contact_hours = $ainew_options['contact-hour'];
	if ($contact_hours){
...
function address_func( $atts = array(), $content = '' ) {
	...
	if ($atts['index']) :
	    ...
	    $html .= '<span class="address-title">'.$ainew_options['contact-address'][$i]['title'].'</span>';
		if ($ainew_options['contact-address'][$i]['map_link']) :
			$html .= '<a class="address-details" href="'.$ainew_options['contact-address'][$i]['map_link'].'" target="_blank">'.$ainew_options['contact-address'][$i]['description'].'</a>';
		else :
			$html .= '<span  class="address-details">'.$ainew_options['contact-address'][$i]['description'].'</span>';
		endif;
	    $html .= '</span>';
	else :
		if(@$ainew_options['contact-address']) :
			foreach ($ainew_options['contact-address'] as $address) :
				if ($n > $atts['offset']) :
				    $html .= '<span class="address address-'.$n.'">';
...
function social_menu_fnc( $atts = array(), $content = '' ) {
	...
	global $ainew_options;
	$html = '';
	$contact_social = $ainew_options['contact-social'];
	$contact_address = $ainew_options['contact-address'];
	$atts = shortcode_atts( array(
		'display' => 'inline',
		'title' => 0,
	), $atts, 'social-menu' );
