		...
		if ('pingback' == $comment->comment_type || 'trackback' == $comment->comment_type) { 
			...
			echo '<div class="media comment_section">';
				echo '<div class="media-body">';
					_e('Pingback:', 'ainew');
					comment_author_link(); 
					edit_comment_link(__('Edit', 'ainew'), '<span class="edit-link">', '</span>');
				echo '</div>';
			echo '</div>';
		} else {
			echo '<li id="comment-';
				...
				echo '<div class="comment-content media-body">';
					echo '<div class="well">';
						echo '<div class="comment-author vcard">';
							...
							// comment author says
							printf(/*__('%s <span class="says">says:</span>', 'ainew'),*/ sprintf('<strong>%s</strong>&nbsp; ', get_comment_author_link()));
							// date-time
							/*echo '<a href="';
							...
							comment_reply_link(array_merge($args, array(
								...
								'reply_text' => '' . __('', 'ainew'),
								'login_text' => '<span class="fa fa-reply"></span> ' . __('Log in to Reply', 'ainew')
							)));
							// end reply link
	
							echo ' ';
	
							edit_comment_link('<span class="fa fa-pencil-square-o "></span>' . __('Edit Comment', 'ainew'), '<span class="edit-link">', '</span>');
							...
							if ('0' == $comment->comment_approved) {
								echo '<div class="comment-awaiting-moderation text-warning"> <span class="glyphicon glyphicon-info-sign"></span> ';
									_e('Your comment is awaiting moderation.', 'ainew');
								echo '</div>';
							} //endif;
...
add_filter('comment_form_defaults', 'ainew_comment_defaults');
function ainew_comment_defaults($defaults) {
	$defaults['comment_notes_before'] = '';
	$defaults['title_reply'] = 'Leave a comment';
	$defaults['title_reply_to'] = 'Leave a comment %s';
	$defaults['label_submit']  = __( 'Submit Comment', 'ainew' );
	return $defaults;
}
