...
$class = $ainew_options['sections-content-class'];
$from_theme_option = $ainew_options['general-page-sections'];
$from_page_option = get_post_meta( get_the_ID(), '_ainew_page_section_layout', true );
$sections = (@$from_page_option['Enabled'])?$from_page_option['Enabled']:$from_theme_option['Enabled'];
unset($sections['content']);
?><?php get_header() ?>
<section id="page" class="page-content <?php if(@$ainew_options['sections-content-background-type'] == 1) echo @$ainew_options['sections-content-background'] . ' ';?><?php if(@$ainew_options['sections-content-color-type'] == 1) echo @$ainew_options['sections-content-color'];?> <?php echo $class ?>">
				...
				<?php while ( have_posts() ) : the_post(); ?>
					...
					<div class="container">
						<?php get_template_part( 'content', 'page' ) ?>
							...
							<?php 
							$gallery_images = get_post_meta( get_the_ID(), '_ainew_gallery_images', true );					
							$layout = ( get_post_meta( get_the_ID(), '_ainew_gallery_layout', true ) ) ? get_post_meta( get_the_ID(), '_ainew_gallery_layout', true ) : '6';
							$large_image_size =  get_post_meta( get_the_ID(), '_ainew_large_image_size', true );
							$image_width =  get_post_meta( get_the_ID(), '_ainew_image_width', true );
							$image_height =  get_post_meta( get_the_ID(), '_ainew_image_height', true );
							$image_per_page =  get_post_meta( get_the_ID(), '_ainew_image_per_page', true );
							?>
