...
// Don't duplicate me!
if (!class_exists('ReduxFramework_mos_social')) {
     ...
     * @since       1.0.0
     */
    class ReduxFramework_mos_social {
        ...
        public function render() {
             ...
             $defaults = array(
                ...
                'content_title' => __ ( 'Social Media', 'ainew' )
            );
                ...
                foreach ( $slides as $slide ) {
                   ...
                   //  echo '</div>' . "\n";               
                    ...
                    if ( $this->field[ 'show' ][ 'basic_icon' ] ) {
                        echo '<li><input type="text" id="' . esc_attr( $this->field['id'] ) . '-basic_icon_' . esc_attr( $x ) . '" name="' . esc_attr( $this->field['name'] ) . '[' . esc_attr( $x ) . '][basic_icon]" value="' . esc_attr($slide['basic_icon']) . '" placeholder="'.esc_attr__('Basic Icon/Image', 'ainew').'" class="full-text iconlist" /></li>';
                    }
                    if ( $this->field[ 'show' ][ 'hover_icon' ] ) {
                        echo '<li><input type="text" id="' . esc_attr( $this->field['id'] ) . '-hover_icon_' . esc_attr( $x ) . '" name="' . esc_attr( $this->field['name'] ) . '[' . esc_attr( $x ) . '][hover_icon]" value="' . esc_attr($slide['hover_icon']) . '" placeholder="'.esc_attr__('Hover Icon/Image', 'ainew').'" class="full-text" /></li>';
                    }
                    if ( $this->field[ 'show' ][ 'link_url' ] ) {
                        echo '<li><input type="text" id="' . esc_attr( $this->field['id'] ) . '-link_url_' . esc_attr( $x ) . '" name="' . esc_attr( $this->field['name'] ) . '[' . esc_attr( $x ) . '][link_url]" value="' . esc_attr($slide['link_url']) . '" class="full-text upload" placeholder="'.esc_html__('URL', 'ainew').'" /></li>';
                    }
                    if ( $this->field[ 'show' ][ 'target' ] ) {
                        ...
                        echo ' '.esc_html__('Open Link in New Tab/Window', 'ainew'). '</label></li>';
                    }
            ...
            if ($x == 0) {
                echo '<div class="redux-slides-accordion-group"><fieldset class="redux-field" data-id="'.esc_attr( $this->field['id'] ).'"><h3><span class="redux-slides-header">New Social Media</span></h3><div>';
                ...
                if ( $this->field[ 'show' ][ 'basic_icon' ] ) {
                    $placeholder = (isset($this->field['placeholder']['basic_icon'])) ? esc_attr($this->field['placeholder']['basic_icon']) : __( 'Basic Icon/Image', 'ainew' );
                ...
                if ( $this->field[ 'show' ][ 'hover_icon' ] ) {
                    $placeholder = (isset($this->field['placeholder']['hover_icon'])) ? esc_attr($this->field['placeholder']['hover_icon']) : __( 'Hover Icon/Image', 'ainew' );
                ...
                if ( $this->field[ 'show' ][ 'link_url' ] ) {             
                    $placeholder = (isset($this->field['placeholder']['link_url'])) ? esc_attr($this->field['placeholder']['link_url']) : __( 'URL', 'ainew' );
